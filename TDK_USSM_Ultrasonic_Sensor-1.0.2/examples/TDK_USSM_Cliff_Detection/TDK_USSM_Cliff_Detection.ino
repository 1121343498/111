...
void Chasm_Detection_Application() {
  ...
  TdkUssm.MeasRead(reg, 0);
  
  TdkUssm.GetValOfRealVal(C_LUT_NPULSES, &val, 8);  
  TdkUssm.SetBitsU32(reg, C_MEASUREMENT[NPULSE_A].lsb, C_MEASUREMENT[NPULSE_A].nbit, val); 
  
  TdkUssm.GetValOfRealVal(C_LUT_TMEAS, &val, 8.75);  
  TdkUssm.SetBitsU32(reg, C_MEASUREMENT[TMEAS_A].lsb, C_MEASUREMENT[TMEAS_A].nbit, val); 
  
  TdkUssm.MeasWrite(reg, 0);
 
  //-- Write initial configuration for CALIB register
  // V_DRV = default V_DRV + 1; G_ANA = default G_ANA + 1; G_DIG = default G_DIG
  TdkUssm.CalibRead(reg, 0); 

  //-- G_ANA
  TdkUssm.SetBitsU32(reg, C_CALIBRATION[G_ANA].lsb, C_CALIBRATION[G_ANA].nbit, 4);  
  
  TdkUssm.CalibWrite(reg, 0);
  ...
  while(true) {
    ...
    for (int i = 0; i < num_adc_samples; i++) {
      if(ts*i*1E-4*343/2 >= detection_window[0] && ts*i*1E-4*343/2 < detection_window[1] && values[i] > threshold[i]) {
        ...
        //-- Reset G_ANA & G_DIG to original values & write to CALIB register
        TdkUssm.CalibRead(reg, 0);
        
        TdkUssm.SetBitsU32(reg, C_CALIBRATION[G_ANA].lsb, C_CALIBRATION[G_ANA].nbit, 4);
        TdkUssm.SetBitsU32(reg, C_CALIBRATION[G_DIG].lsb, C_CALIBRATION[G_DIG].nbit, 0);  
    ...
    if(!trigger) {
      ...
      if (missing_count < missing_reference && missing_count % 2 == 1) {
        ...
        val = TdkUssm.GetBitsU32(reg, C_CALIBRATION[G_ANA].lsb, C_CALIBRATION[G_ANA].nbit);
        if(val<7)  val++;
        TdkUssm.SetBitsU32(reg, C_CALIBRATION[G_ANA].lsb, C_CALIBRATION[G_ANA].nbit, val);  
        ...
        val = TdkUssm.GetBitsU32(reg, C_CALIBRATION[G_DIG].lsb, C_CALIBRATION[G_DIG].nbit);
        if(val<127)  val = val + 3;
        TdkUssm.SetBitsU32(reg, C_CALIBRATION[G_DIG].lsb, C_CALIBRATION[G_DIG].nbit, val);  
